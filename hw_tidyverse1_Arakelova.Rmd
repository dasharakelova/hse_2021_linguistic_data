...
1.  How many participants are mentioned in this dataframe? 24 participants.
2.  How many of them are males and females? 8 female participants and 16 male participants.
3.  How many first languages are mentioned in the dataframe? 9 languages
...
Let's choose only columns without NAs.
...
```{r}
biling_speech_data$age_of_arrival_to_Canada_months = round(biling_speech_data$age_of_arrival_to_Canada_months/12, digits = 1)

biling_speech_data$age_of_English_exposure_months = round(biling_speech_data$age_of_English_exposure_months/12, digits = 1)

biling_speech_data$age_at_recording_months = round(biling_speech_data$age_at_recording_months/12, digits = 1)

biling_speech_data$months_of_english = round(biling_speech_data$months_of_english/12, digits = 1)
```

### 2.3 Rename changed columns for convenience

```{r}
biling_new = biling_speech_data %>%
  rename(age_of_arival_years = age_of_arrival_to_Canada_months, age_of_english_exposure_years = age_of_English_exposure_months, age_years = age_at_recording_months, years_of_english = months_of_english)
```
...
1.  What is the average age of child migration to Canada? 3.7 years.

```{r}
biling_new %>%
  summarise(mean(age_of_arival_years))
```

2.  How many children whose first language is Spanish learnt English less than 10 month? How many of them are males and females? 3 children, all of them male.

```{r}
biling_new %>%
  filter(first_language == 'Spanish' & years_of_english < 0.8) %>%
  count((gender))
```
...
biling_new %>%
  ...
  summarise(mean(age_years), mean(age_of_arival_years))
```
...
Not sure why add the information about the first language cause there's no point in grouping then.

```{r}
biling_new %>%
  group_by(gender) %>%
  summarise(first_language, across(age_of_english_exposure_years, c(mean, min, max)))
```
...
```{r}
main = read_csv('main_data.csv')
cds_freq = read_csv('Norwegian_CDS_frequency.csv')
```
...
```{r}
main = main %>%
  select(Translation:CDS_freq & !(Lex_cat | Freq))

cds_freq = cds_freq %>%
  select(Translation:Freq_CDS)
```
...
```{r}
freq_statistics %>%
  pivot_longer(cols = CDS_freq:Freq_NoWaC, names_to = 'metrics', values_to = 'entries')
```
